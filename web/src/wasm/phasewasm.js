...
export class WasmEngine {
     ...
     * @returns {any}
     */
    integrate_scene(scene_json) {
        const ptr0 = passStringToWasm0(scene_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.wasmengine_integrate_scene(this.__wbg_ptr, ptr0, len0);
        if (ret[2]) {
     ...
     * @returns {string}
     */
    default_lorenz_scene() {
        let deferred1_0;
        let deferred1_1;
        try {
            const ret = wasm.wasmengine_default_lorenz_scene(this.__wbg_ptr);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('phasewasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
