...
import { ViewerProvider, useViewerState } from "../state/viewerState";
import { getBackgroundColors } from "../theme/backgroundModes";

function PhaseViewerContent() {
  const { background, customBackgrounds } = useViewerState();
  const { page } = getBackgroundColors(background, customBackgrounds);

  return (
    <div className="flex min-h-screen flex-col" style={{ background: page }}>
      ...
      <section id="about" className="bg-[color:var(--ps-bg)] py-12 text-[color:var(--ps-text)]">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4">
          ...
          <p className="max-w-3xl text-sm text-[color:var(--ps-text-soft)]">
            phase-space is a lightweight viewer that combines WebAssembly integrations with WebGL rendering. Use it to inspect trajectories,
            experiment with parameters, and enjoy smooth interactions across devices.
          </p>
        </div>
      </section>
    </div>
